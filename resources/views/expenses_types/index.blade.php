...
?>
@extends('layouts.app')
@section('title','إدارة أقسام المصروفات')
...
@section('content')
    <main dir='rtl' class='pt-4 px-md-3 pb-2'>
        <section class='animated fadeInDown ml-auto faster table-filters'>
            <h1 class='text-white text-center font-weight-bold pb-3 mh1'>إدارة أقسام المصروفات</h1>
            <div class='px-3'>
        ...
        <section class='edit d-none'>
            <div class='w-100 h-100 position-fixed' style='left:0;top: 0;z-index: 2;background: rgba(113, 107, 107, 0.5);'>
                <div id='formEdit' class='container text-center box-shadow  bg-white pb-2'>
                    <form id='editSaves' action='{{route('expenses_types.update',0)}}' method='post'>
                        @csrf
                        @method('put')
                        <fieldset class="px-1" style="border: 3px solid #164e5b">
        ...
        {{--delete saves--}}
        <form class='d-inline' id='deleteSaves' action='{{route('expenses_types.destroy',0)}}' method='post'>
            @method('delete')
            @csrf
        </form>
        {{--changeState saves--}}
        <form class='d-inline' id='changeStateSaves' action='{{route('expenses_types.changeState',0)}}' method='post'>
            @csrf
        </form>
    </main>
@endsection
...
@section('js')
    <script defer>
        ...
        function addSaves() {
            ...
            $.ajax({
                url: '{{route('expenses_types.store')}}',
                method: 'POST',
                data: {
                    name: name,
                },
                ...
                error: function (e) {
                    design.useSound('error');
                    alertify.error("من فضلك تحقق من إسم القسم");
                    console.log(e);
                    return;
                }
            });
        ...
        function getData() {
            $('#name').val('');
            $.ajax({
                url: '{{route('expenses_types.getDate')}}',
                method: 'POST',
                success: function (data) {
