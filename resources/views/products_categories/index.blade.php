...
@section('content')
    <main dir='rtl' class='pt-4 px-md-3 pb-2'>
        <section class='animated fadeInDown ml-auto faster table-filters'>
            <h1 class='text-white text-center font-weight-bold pb-3 mh1'>إدارة أقسام المنتجات</h1>
            <div class='px-3'>
                <div class='container text-center box-shadow bg-white pb-2'>
                    <fieldset class="px-1" style="border: 3px solid #164e5b">
                        <legend class="w-auto px-2">إضافة قسم جديد</legend>
                        <div class='form-group row h3'>
                            <label class='col-sm-3 pt-2 text-md-left '>إســم القسم</label>
                            <div class='col-sm-6'>
                                <input value=''
                                       type='text'
                                       data-filter-col="1,2"
                                       ...
                                       placeholder='من فضلك أدخل إسم القسم المراد إضافتة'
                ...
                <div class='mt-2'>
                    <div class="input-group">
                        ...
                        <input type='text' id="search"
                               data-filter-col="1,2"
                               placeholder='ابحث عن قسم' class='form-control'>
                    </div>
                </div>
        ...
        <section class='edit d-none'>
            <div class='w-100 h-100 position-fixed' style='left:0;top: 0;z-index: 2;background: rgba(0, 0, 0, 0.7);'>
                <div id='formEdit' class='container text-center box-shadow  bg-white pb-2'>
                    <form id='editSaves' action='{{route('products_categories.update',0)}}' method='post'>
                        @csrf
                        @method('put')
                        <fieldset class="px-1" style="border: 3px solid #164e5b">
                            <legend class="w-auto px-2">تعديل قسم
                                <span id='oldName'></span>
                            </legend>
                            <div class='form-group row h3'>
                                <label class='col-sm-2 pt-2 text-md-left '> إسم القسم الجديد</label>
                                <div class='col-sm-5'>
                                    <input value='' required
                                           type='text'
                                           id='newName'
                                           data-validate='min:5' data-patternType='min'
                                           placeholder='من فضلك أدخل إسم القسم الجديد'
                                           name='name' class='form-control pr-5'>
                                </div>
        ...
        {{--delete saves--}}
        <form class='d-inline' id='deleteSaves' action='{{route('products_categories.destroy',0)}}' method='post'>
            @method('delete')
            @csrf
        </form>
        {{--changeState saves--}}
        <form class='d-inline' id='changeStateSaves' action='{{route('products_categories.changeState',0)}}' method='post'>
            @csrf
        </form>
    </main>
@endsection
...
@section('js')
    <script defer>
        ...
        function addSaves() {
            var name = $('#name').val();
            if (name.trim() == '') {
                alertify.error("من فضلك إدخل إسم القسم المراد إضافته");
            ...
            $.ajax({
                url: '{{route('products_categories.store')}}',
                method: 'POST',
                data: {
                    name: name,
                },
                ...
                error: function (e) {
                    alertify.error("من فضلك تحقق من إسم القسم قد يكون هناك قسم بنفس الإسم");
                    console.log(e);
                    design.useSound('error');
                    return;
                }
        ...
        function getData() {
            $('#name').val('');
            $.ajax({
                url: '{{route('products_categories.getDate')}}',
                method: 'POST',
                success: function (data) {
        ...
        $('table').on('click', 'tbody button[data-type="delete"]', function (e) {
            ...
            $(this).confirm({
                text: "هل تريد حذف القسم المحدد " + name,
                title: "حذف قسم منتجات ؟ ",
