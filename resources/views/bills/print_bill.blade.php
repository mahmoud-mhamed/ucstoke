...
@section('content')
    <main dir='rtl' class='pt-4 px-3 pb-2'>
        <div>
            @foreach($print_dismissal_notice?[1,2]:[1] as $print_item)
                @if ($design->use_small_size==0)
                    <section id="section_bill_design"
                             class='mx-auto font-ar pt-3 pr-3 pl-3 pb-0 bg-white text-center position-relative'
                        ...
                        <div class='row no-gutters'>
                            <div class='col-7 text-right font-weight-bold'
                                 style="font-size: {{$design->header_size}}rem!important;">
                                <img height='80px'
                            ...
                            <div class='col-5 text-left font-weight-bold'
                                 style="font-size: {{$design->bill_number_date_size}}rem!important;">
                        ...
                        @if($print_item===1)
                            @if ($bill->type==0)
                                <p class="font-weight-bold" style="font-size: {{$design->header_size}}rem!important;">
                                    فاتــورة شــــراء</p>
                            @endif
                        @endif
                        @if($print_item===2)
                        ...
                        <table class='w-100 mt-2' border='3' id="tableData">
                            <thead>
                            <tr class=''
                                style="background: #e4e4db;font-size: {{$design->table_header_size}}rem!important;">
                        ...
                        <div class='pt-1 m-0 font-en font-weight-bold overflow-hidden'
                             style="font-size: {{$design->message_uc_size}}rem!important;">
                ...
                @else
                    <section id="section_small_bill_design"
                             style="width: {{$design->small_size}}cm!important;margin-left: auto;margin-right: auto;padding:0.25rem!important;text-align: center;"
                             class='mx-auto small_design font-ar p-1 pb-0 bg-white text-center'>
                        <div class=''>
                            <div class='text-right font-weight-bold'
                                 style="text-align:right;font-weight: bolder;background: #d5d5d5;border-radius: 4px">
                                <div class='d-block text-center font-weight-bold'
                                     style="display:block;text-align: center;font-weight: bolder;font-size: {{$design->header_size}}rem!important;">
                                    <img
                                        class='' id="img_design1" src='{{$design->icon}}'
                                        style='max-width:19%;vertical-align: top;float: right;'>
                                    <p id="p_design_company_name1" class="mb-0"
                                       style="margin-bottom: 0;">{{$design->company_name}}</p>
                                    <p id="p_design_row_under_company_name1"
                                       class="mb-0 mt-0"
                                       style="margin-bottom: 0">{{$design->row_under_company_name}}</p>
                                </div>
                                <div style="clear: both"></div>
                            </div>
                        </div>
                        <div class='font-en py-1 text-right'
                             style="padding-top: 0.25rem!important;padding-bottom: 0.25rem!important;text-align: right;font-size: {{$design->contact_size}}rem!important;">
                            <div class='text-right small_bold' style="text-align: right;"
                                 id="div_row_contact11">{{$design->row_contact1}}</div>
                            <div id="div_row_contact21" style="text-align: right"
                                 class='text-right small_bold'>{{$design->row_contact2}}</div>
                        </div>
                        @if ($bill->type==0)
                            <p class="font-weight-bold"
                               style="font-weight: bolder;font-size: {{$design->header_size}}rem!important;">فاتــورة
                                شــــراء</p>
                        @endif
                        @if (isset($bill->account->name))
                            <div class='text-right small_bold px-1'
                                 style="text-align: right;font-weight: bolder;padding-right: 0.25rem;padding-left: 0.25rem!important;font-size: {{$design->account_size}}rem!important;">
                                إسم
                                {{$bill->type==0?'المورد':'العميل'}}
                                :
                        ...
                        <div class='text-left mb-0  px-1'
                             style="text-align: left;margin-bottom: 0px;padding-right: 0.25rem!important;padding-left: 0.25rem!important;font-size: {{$design->bill_number_date_size}}rem!important;">
                        ...
                        <div class='pt-1 m-0 font-en small small_bold overflow-hidden'
                             style='padding-top: 0.25rem!important;font-size: {{$design->message_uc_size}}rem!important;'>
                            <p class='text-left float-left p-0 m-0 text-dark'
                               style="text-align: left;float: left;padding: 0;margin: 0;"><span
                                    style='text-align: left;background: transparent'>{{$bill->message}}</span></p>
                            <p class='text-right float-right p-0 m-0 text-dark'
                               style="text-align: right;float: right;padding: 0;margin: 0;">
                            <span
                ...
                @endif
                @if($print_dismissal_notice && $print_item === 1)
                    {!! $new_page !!}
                @endif
            @endforeach
        </div>
